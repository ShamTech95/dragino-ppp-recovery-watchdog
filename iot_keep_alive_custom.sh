...
# Guarded cellular recovery state machine
gsm_fail_count=0
ppp_recovery_attempt=0
recovery_round=0
cooldown_until=0
last_auto_reboot=0
...
GSM_FAILURE_LIMIT=5
PPP_ATTEMPTS_PER_ROUND=3
PPP_STABILIZE_SECONDS=120
COOLDOWN_ROUND_1=1800
COOLDOWN_ROUND_2=3600
COOLDOWN_ROUND_3=7200
...
# Load the last automatic-reboot timestamp, if available.
...
reset_cellular_recovery_state()
{
	if [ "$cellular_state" != "NORMAL" ] || [ "$gsm_fail_count" -ne 0 ] || \
	   [ "$ppp_recovery_attempt" -ne 0 ] || [ "$recovery_round" -ne 0 ] || \
	   [ "$cooldown_until" -ne 0 ]; then
		logger -t iot_keep_alive "Cellular connectivity restored; recovery state reset"
	fi
	gsm_fail_count=0
	ppp_recovery_attempt=0
	recovery_round=0
...
start_cellular_cooldown()
{
	...
	ppp_recovery_attempt=0
	gsm_fail_count=0
	case "$recovery_round" in
		1) cooldown_seconds="$COOLDOWN_ROUND_1" ;;
		2) cooldown_seconds="$COOLDOWN_ROUND_2" ;;
		3) cooldown_seconds="$COOLDOWN_ROUND_3" ;;
		*) cooldown_seconds="$COOLDOWN_ROUND_3" ;;
	esac
	cooldown_until=$((current_time + cooldown_seconds))
	cellular_state="COOLDOWN"
	logger -t iot_keep_alive "Cellular recovery round $recovery_round failed; cooldown for $cooldown_seconds seconds"
}
...
request_guarded_gateway_reboot()
{
	...
	if [ "$elapsed_time" -ge "$AUTO_REBOOT_GUARD" ]; then
		...
		logger -t iot_keep_alive "All cellular recovery rounds failed; guarded gateway reboot authorised"
		sync
		sleep 3
		reboot
		return
	...
	ppp_recovery_attempt=0
	recovery_round=0
	cooldown_until=$((current_time + remaining_time))
	cellular_state="COOLDOWN"
	logger -t iot_keep_alive "Automatic reboot blocked by six-hour guard; cooldown for $remaining_time seconds"
}

perform_ppp_recovery()
{
	ppp_recovery_attempt=$((ppp_recovery_attempt + 1))
	cellular_state="PPP_RECOVERY"
	gsm_fail_count=0
	logger -t iot_keep_alive "Starting PPP recovery attempt $ppp_recovery_attempt of $PPP_ATTEMPTS_PER_ROUND in round $((recovery_round + 1))"
	...
	logger -t iot_keep_alive "PPP reconnect initiated; waiting $PPP_STABILIZE_SECONDS seconds"
	sleep "$PPP_STABILIZE_SECONDS"
	check_3g_connection
	if [ "$gsm_ping" -gt "$ZERO" ]; then
		reset_cellular_recovery_state
		return
	fi
	logger -t iot_keep_alive "PPP recovery attempt $ppp_recovery_attempt failed"
	if [ "$ppp_recovery_attempt" -ge "$PPP_ATTEMPTS_PER_ROUND" ]; then
		if [ "$recovery_round" -ge 3 ]; then
			request_guarded_gateway_reboot
		else
			start_cellular_cooldown
		fi
...
cellular_recovery_tick()
{
	...
	case "$cellular_state" in
		NORMAL)
			...
			logger -t iot_keep_alive "Cellular connectivity failure $gsm_fail_count of $GSM_FAILURE_LIMIT"
			if [ "$gsm_fail_count" -ge "$GSM_FAILURE_LIMIT" ]; then
				perform_ppp_recovery
			fi
			;;
		PPP_RECOVERY)
			if [ "$ppp_recovery_attempt" -lt "$PPP_ATTEMPTS_PER_ROUND" ]; then
				perform_ppp_recovery
			elif [ "$recovery_round" -ge 3 ]; then
				request_guarded_gateway_reboot
			else
				start_cellular_cooldown
			fi
		...
		COOLDOWN)
			if [ "$current_time" -ge "$cooldown_until" ]; then
				cellular_state="PPP_RECOVERY"
				ppp_recovery_attempt=0
				logger -t iot_keep_alive "Cellular recovery cooldown complete; starting round $((recovery_round + 1))"
				perform_ppp_recovery
			fi
			;;
		REBOOT_PENDING)
			...
			;;
		*)
			logger -t iot_keep_alive "Unknown cellular recovery state '$cellular_state'; resetting"
			reset_cellular_recovery_state
			;;
	esac
}
...
check_3g_connection()
{
	# Connectivity-only check. Do not touch the modem AT port or terminate comgt.
	gsm_ping="$ZERO"
	GSM_IF=$(ifconfig 2>/dev/null | awk '/^3g-/{print $1; exit}')
	[ -z "$GSM_IF" ] && return
	gsm_ip=$(ifconfig "$GSM_IF" 2>/dev/null | awk -F'[: ]+' '/inet /{print $4; exit}')
	[ -z "$gsm_ip" ] && return
	logger -t iot_keep_alive "Ping GSM $gsm_ip via $GSM_IF"
	gsm_ping=$(fping -I "$GSM_IF" "$PING_HOST" 2>/dev/null | grep -c alive)
	if [ "$gsm_ping" -eq "$ZERO" ]; then
		gsm_ping=$(fping -I "$GSM_IF" "$PING_HOST2" 2>/dev/null | grep -c alive)
	fi
	[ "$gsm_ping" -gt "$ZERO" ] && logger -t iot_keep_alive "GSM Cellular is alive"
	GSM_GW=$(ip route show | grep "$GSM_IF" | awk '{print $1; exit}')
}
...
do
	...
	0)
		...
		# Keep the factory 15-minute reboot only when cellular recovery is disabled.
		if [ "$cellular_auto" != "1" ] && \
		   { [ "$internetdetect" = "checked" ] || [ -z "$internetdetect" ]; }; then
			cur_flag_time=$(date +%s)
			if [ -z "$has_internet_flag_time" ]; then
				has_internet_flag_time="$cur_flag_time"
			else
				time_diff=$((cur_flag_time - has_internet_flag_time))
				if [ "$time_diff" -gt 900 ] && [ "$time_diff" -le 3000 ]; then
					reboot
				fi
			fi
		fi

		# Base recovery on cellular reachability, not a possibly stale IoT status.
		if [ "$cellular_auto" = "1" ] && \
		   { [ "$server_type" = "lorawan" ] || [ "$server_type" = "station" ]; }; then
			cellular_recovery_tick
		elif [ "$iot_online" = "0" ]; then
			logger -t iot_keep_alive "No Internet Connection and IoT Service offline"
		else
			logger -t iot_keep_alive "No Internet Connection but IoT Service Online"
		fi
